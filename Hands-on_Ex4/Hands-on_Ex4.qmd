...
We will discover how to see and analyze the correlation of the input variables using the corrplot.mixed() function of the corrplot package.
...
The following step is to replace row number with township name in the rows and delete the TS.x field by selecting only the required columns (2 to 6)
...
The Base R function `scale()` makes standardizing Z-scores simple. The Z-score approach will be used to standardize the clustering variables below. We use the `describe()` function of the **psych** package here because we want to look at the standard deviation of the variable
...
It performs similar operations to `hclus()`, but `agnes()` also provides the agglomerative coefficient, which gauges the degree of clustering structure present (values closer to 1 suggest strong clustering structure).
