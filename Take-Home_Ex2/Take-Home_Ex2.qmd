...
48.29% are functional, 4.82% are functional but needs repair and 1.77% of them are functional but not in use.

To aggregate data, Abandoned and Decommissioned water points were be grouped as non functional as well.
...
Checking out the quality of water points, we know that 75.57% of them are of acceptable quality, with an additional 0.04% of them within potable national limits / standards. We will aggregate Acceptable Quality, Within National Standards and Within National Limits as Potable water for this analysis.

The rest are either unknown or of unacceptable quality.

```{r}
freq(data=nga_wp, input = 'Water_Source_Category')
```

Checking out the source of water points, we know that majority of them (96.35%) comes from well. With such a high number, this variable may not be useful for our analysis as most of the data points will have this value.
...
After we are done, we will inspect the data by viewing the data frame, we can observe that there are some rows that have NaN values due to division by zero. We will replace these NaN values to 0 with the `is.na()` function and change the row names to the names of the LGAs with `row.names()` by using the code below
...
We will only keep the following variables as the rest are intermediate data points that are no longer relevant by using `select()` and verify that the data is columns are correct using `colnames()`
...
We can see that there isn't any outliers in the functional water points, but for the non functional water points, there are outliers where 100% of the water points are not functional.

In the figure below, multiple histograms are plotted to reveal the distribution of the selected variables in the *nga_wp_interested_data_pts* data.frame. First, We do this by creating all the histograms assigned to individual variables by using `ggplot()`.
...
We can build a trellis plot with the code below after creating our graphs with `ggarrange()` to create 14 histograms. and organised into a 3 columns by 5 rows small multiple plot.
...
> *As functional and non functional water points are **NOT** of the same units as the other variables in percentage, we will need to standardize them later.*
...
As a result, the following **5 observations** are strongly correlated with the findings.

1.  *pct_functional* vs. pct_managed - we will drop *pct_managed*

2.  pct_handpump vs. *pct_Cap_Over1000* and *pct_Cap_Under1000* - we will drop *pct_Cap_Under1000* and *pct_handpump*

3.  *pct_potable* vs. *pct managed* - *pct_managed* already drop in 1

4.  *pct_Cap_Over_1000* vs. *pct_Cap_Under1000* - *pct_Cap_Under1000* already drop in 2

5.  *pct_w\_meddist_to_PriRoad* vs. *pct_w\_meddist_to_TerRoad* - we will drop *pct_w\_meddist_to_TerRoad*

In conclusion, we will drop *pct_handpump (Column 5), pct_managed (Column 10), pct_Cap_Under1000 (Column 8), and pct_w\_meddist_to_TerRoad (Column 14).* Using `select()`, we drop the respective columns.
...
As most of the data points are already in percentages except *wpt functional* and *wpt non functional*, we will need to perform standardization so that they will be comparable. From the Trellis plot generated earlier, we know that not all of the variables follow some normal distribution, so we will use ***Min-Max*** standardization instead of the Z-score standardization
...
We set k.max to 10 as there are 10 variable involved and we want to find the global maximum to determine what is the best value of *k.*
...
Following a thorough analysis of the dendragram shown above, we chose to keep 9 groups. The code below will use R Base's `cutree()` function to create a 9-cluster model.
...
We can see that without spatial constraints, the map generated 9 clusters that are quite messy and the regions forming the clusters are all over the place. This is sub optimal as there may be real world consideration that necessitates the regionalization of geographies into distinct regions, such as urban planning.
...
As there is an island involved, we will build an adaptive weight matrix.
...
Finally, we can also plot the pruned tree that shows the 9 clusters on top of the LGA area.
...
First we will need to create the *`nga_clustgeo_HClust`* using `hclustgeo()` with the proximity matrix we created in the previous section and plot its corresponding Dendrogram
...
Without spatial constraints, the map generated 9 clusters that are rather messy and the regions composing the clusters are dispersed throughout the map, which is sub-optimal. This is similar to the results we applied in the section *Mapping the clusters formed in hierarchical clustering*.
...
We can observe that when ***∝*** is raised from 0.2 to 0.4, the homogeneity of d1, which is our spatial relationship (contiguity matrix), gain significantly from about 50% to 80%. And for d0 (which is our attribute), the homogeneity loss is slightly more than 20%. Since ***∝ = 0.4*** is the most optimal, we will use it to form our cluster.
...
Based on the results, we can see that hierarchical clustering without spatial constraints produces clusters that are messy with no contiguity which is sub optimal in both methods if resources must be focused among similar areas. Such clustering can posed to be difficult when one conduct planning without spatial considerations.

We can see that the SKATER map generated a map that assumed that the bulk of the water points were homogeneous, creating a relatively large cluster (Cluster 4). Due to the size of the cluster, this may not be the right approach for this data set. We will investigate the reasons in the next section
...
The box plot discloses The highest median for percentage non functional water points is shown by Cluster 5, then come cluster 1, 4, 9, 8, 3, 7, 2, 6. The clustering algorithm correctly identified Cluster 6 to the North East of Nigeria, where there are very few water points.

In the previous section, we noted that the SKATER map produced a really large cluster: cluster 4. Lets investigate what are the contributing factors. We first filter the rows in cluster 4, and plot the multi variable box plot using `ggparcoord()`
...
From the result, we see that the largest contributing factor to this trend is whether the area is rural or otherwise, followed by having potable water and a capacity of less than 1000. We could infer that the clustering algorithm has grouped these similar rural areas that had potable water and capacity of less than 1000 together. The plot also identified that the cluster has more functional than non functional water points in cluster 4, by a thin margin.

The result of cluster 4 which spans 388 LGAs is unlikely to be homogeneous if we want to look at how non-functional water points are distributed, as it does not provide sufficient insights. In order to look for more insights, lets take a look at the results produced by the ClustGeo package.
...
The box plot discloses The highest median for percentage non functional water points is shown by Cluster 4, then come cluster 6, 3, 5, 1, 8, 7, 9, 2. Interestingly, the area identified in cluster 2 has no water points at all.

The result also correctly identified the following, consistent with our observations in [*Visualizing the spatial distribution of water points*](Visualizing the spatial distribution of water points)

-   that Non functional water points congregates around the south western coast of Nigera, facing gulf of Guinea & Bight of Benin which are cluster 4 and 5 respectively
...
Since we know cluster 4 has the highest percentage of non functional water points, lets first filter the rows in cluster 4, and plot the multi variable box plot using `ggparcoord()`
...
According to the results of cluster 4, which is primarily made up of rural areas

-   Half of the LGAs in this cluster have at least 53% non-operational water points in its area.

-   51.53% of the water points are within the median distance of a secondary road

-   At least half of the LGAs in this cluster contains at least 62.39% of the water points with potable water. However, the median of percentage functional water point is only at 39.25%.

    This represent a gap of 23.14% that could serve the population. Even though their capacity may be less than 1000, it could potentially help spread out the people trying to reach water if all of the water points that contains potable water can be made functional.

    We could further examine if distance to reach the water point is an issue to the population.

    Since we know from the correlation matrix (and logically) that functional water points are likely to be managed and contain potable water, we can also investigate if the distance to a road network is preventing maintenance work from going on.

-   The median of Low capacity water points stands at 24.5% here.

#### Cluster 5

To make our analysis complete, lets also look at cluster 5, the adjacent cluster that is facing the Bight of Benin if we can spot similar trends

Similarly, we will filter the rows in cluster 5, and plot the multi variable box plot using `ggparcoord()`
...
According to the results of cluster 5

-   Cluster 5 like cluster 4 is primarily made up of rural areas.

-   The median of percentage functional water point stands at 39.60%, but the difference here is that there are much more potable water points in this cluster at 69.21%. The gap is larger here at 30%.

-   Cluster 5 is also very more much accessible with a median of 72.49% of them having access to a secondary road as compared to cluster 4 at 51.53%

-   Cluster 5 also have a higher number of water points that are small with capacity less than 1000 at 68.0%, significantly higher than the numbers in cluster 4 (24.5%)

#### Cluster 7

We also learnt that the north of Nigeria contains a high number of functional water points, and it is highlighted as cluster 7, Lets try and see if we could spot trends that made it possible.

Similarly, we will filter the rows in cluster 7, and plot the multi variable box plot using `ggparcoord()`
...
As we would expect, there are a high number of functional water points in the cluster.

-   Cluster 7 is also primarily made up of rural area, we could hence infer that urbanization does not really contribute to whether a water point is functional or not

-   90% of the water points in this cluster are small, as compared to cluster 4 (24.5%) and cluster 5 (68%), we might investigate if a higher number of smaller water points are the way to go instead of big ones. We might also want to probe if large water points are harder to maintain.

-   A very high number of water points (median of 94.44%) contains potable water. The median of percentage functional water points stands at 78.05%. The gap here is only 16.39%, lower than that of Cluster 4 and 5.

-   42.13% of the water points are within the median distance of a secondary road

## Conclusion

To further investigate how the variables of interest interact with each other, we could form hypothesis and use regression techniques to prove them.
...
-   Can we find out what variables will affect water point containing potable water to become non functional and how can we make it functional again to serve the population
