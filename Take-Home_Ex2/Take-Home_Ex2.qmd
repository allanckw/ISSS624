...
First we will need to create the nongeo_cluster using `hclustgeo()` with the proximity matrix we created in the previous section and plot its corresponding Dendrogram
...
The box plot discloses The highest median for percentage non functional water points is shown by Cluster 5, then come cluster 1, 4, 9, 8, 3, 7, 2, 6. Cluster 6 has little to no water points in the SKATER Plot. The clustering algorithm correctly identified Cluster 6 & 8 to the North East of Nigeria, where there are very few water points.
...
However, the plot does not give us sufficient insights as the area is too large which covers 388 LGAs, which is unlikely to be homogeneous if we would like to investigate how non functional water points are distributed. With that, we shall look at the results generated by the ClustGeo package to investigate how the clusters are formed
...
The box plot discloses The highest median for percentage non functional water points is shown by Cluster 4, then come cluster 6, 3, 5, 1, 8, 7, 9, 2. Interestingly, the area identified in cluster 2 has no water points at all, with all the variable values at 0.
...
From the result in cluster 4, they are are mostly made up of rural areas, the total number of water point in these area seems to be low with the median non functional water point at
